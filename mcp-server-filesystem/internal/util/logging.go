...
func (aw *AsyncWriter) Write(p []byte) (n int, err error) {
	...
	case <-aw.ctx.Done():
		return 0, aw.ctx.Err() // Context cancelled, escape hatch to prevent deadlock
	}
}
...
func OpenHardenedLogFile(path string) *os.File {
	...
	if info, err := os.Stat(path); err == nil && info.Size() > maxLogSize {
		if err := os.Truncate(path, 0); err != nil {
			if _, writeErr := io.WriteString(os.Stderr, "mcp-server: failed to truncate log file\n"); writeErr != nil {
				// Failed to even write to stderr, fallback is impossible
			}
		}
	}
...
func (f *LogFanout) Write(p []byte) (int, error) {
	if _, err := f.Buffer.Write(p); err != nil {
		// Buffer writes shouldn't typically fail, but ignore synchronously to prioritize file Async
	}
...
func SetupStandardLogging(ctx context.Context, serverName string, buffer io.Writer) func() {
	...
	return func() {
		if err := localAw.Close(); err != nil {
			slog.Error("failed to close async log writer")
		}
		...
		if globalAw != nil {
			if err := globalAw.Close(); err != nil {
				slog.Error("failed to close global async writer")
			}
		}
		if globalLogFile != nil && globalLogFile != os.Stderr {
			if err := globalLogFile.Close(); err != nil {
				slog.Error("failed to close global log file")
			}
		}
	}
}
