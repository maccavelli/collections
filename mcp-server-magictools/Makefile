...
.PHONY: all build clean test run install version build-all linux darwin-amd64 darwin-arm64 help fmt vet lint vendor

all: help build-all

build: ## Compiles the Go application for the local OS/Arch
	@mkdir -p $(DIST_DIR)
	@CGO_ENABLED=0 go build -tags netgo -ldflags "-extldflags '-static' -w -s -X main.Version=$(VERSION)" -o $(DIST_DIR)/$(BINARY_NAME)-$(shell go env GOOS)-$(shell go env GOARCH) .
...
windows-arm64: ## Compiles for Windows ARM64
	@mkdir -p $(DIST_DIR)
	GOOS=windows GOARCH=arm64 go build -ldflags "-X main.Version=$(VERSION)" -o $(DIST_DIR)/$(BINARY_NAME)-windows-arm64.exe .
...
run: build ## Builds and executes the local binary
	@BIN_NAME=$(DIST_DIR)/$(BINARY_NAME)-$(shell go env GOOS)-$(shell go env GOARCH) ; \
	$$BIN_NAME
