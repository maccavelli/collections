...
func TestHardenedAddTool(t *testing.T) {
	...
	sp := &mockSessionProvider{srv}
	HardenedAddTool(sp, tool, handler)
}

type mockSessionProvider struct{ s *mcp.Server }

func (m *mockSessionProvider) MCPServer() *mcp.Server      { return m.s }
