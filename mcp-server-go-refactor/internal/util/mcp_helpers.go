...
func HardenedAddTool[In any, Out any](
	...
	mcp.AddTool(s, tool, func(ctx context.Context, req *mcp.CallToolRequest, input In) (res *mcp.CallToolResult, output Out, err error) {
		defer func() {
			if r := recover(); r != nil {
				slog.Error("CRITICAL: Tool handler panic recovered", "error", r, "stack", string(debug.Stack()))
				res = &mcp.CallToolResult{
					IsError: true,
					Content: []mcp.Content{
						&mcp.TextContent{
							Text: fmt.Sprintf("Internal Error: Tool handler panic recovered: %v", r),
						},
					},
				}
				// We return nil error to the SDK to keep the JSON-RPC pipe open
				// and send the structured error result back to the host.
				err = nil
			}
		}()
