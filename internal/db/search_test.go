...
func TestSearchEvolution(t *testing.T) {
	...
	results, _ = store.SearchTools("find", "search", "", 0.0)
	if len(results) == 0 || results[0].URN != "urn:ddg:search" {
	...
	results, _ = store.SearchTools("find", "filesystem", "", 0.0)
