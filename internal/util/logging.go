...
func (aw *AsyncWriter) Write(p []byte) (n int, err error) {
	...
	case <-aw.ctx.Done():
		atomic.AddInt64(&aw.dropped, 1)
		return len(p), nil // Context cancelled, dropping gracefully
...
func OpenHardenedLogFile(path string) *os.File {
	...
	if info, err := os.Stat(path); err == nil && info.Size() > maxLogSize {
		if err := os.Truncate(path, 0); err != nil {
			if _, wErr := io.WriteString(os.Stderr, "mcp-server: failed to truncate log file "+path+": "+err.Error()+"\n"); wErr != nil {
				fmt.Printf("logging bootstrap failure: %v\n", wErr)
			}
		}
	}
...
func SetupStandardLogging(serverName string, buffer io.Writer) func() {
	...
	return func() {
		if err := localAw.Close(); err != nil {
			slog.Error("failed to close async writer", "error", err)
		}
		...
		if globalAw != nil {
			if err := globalAw.Close(); err != nil {
				slog.Error("failed to close global async writer", "error", err)
			}
		}
		if globalLogFile != nil && globalLogFile != os.Stderr {
			if err := globalLogFile.Close(); err != nil {
				slog.Error("failed to close global log file", "error", err)
			}
		}
	}
}
