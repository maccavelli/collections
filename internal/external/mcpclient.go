...
// to external MCP servers (e.g., mcp-server-recall). It mirrors the proven
// architecture from mcp-server-brainstorm and mcp-server-go-refactor, using
// the standards-compliant transport from the MCP spec (2025-03-26) with a
// RecallEnabled circuit breaker for graceful standalone-mode degradation.
...
// circuit breaker to enabled on success. Retries up to 5 times with
// exponential backoff (1s → 16s) before permanently disabling recall
// connectivity. Once connected, blocks until the session closes or the
			...
			case err := <-errCh:
				if err != nil {
					c.logger.Warn("Streamable HTTP session closed unexpectedly (crash detected)",
						"server", "magictools", "url", c.URL, "error", err)
				}
				c.setRecallEnabled(false)
			}
...
func (c *MCPClient) CallDatabaseTool(ctx context.Context, toolName string, arguments map[string]any) string {
	...
	// Prefer structured content for machine-to-machine consumption
...
// at 32KB to prevent orchestrator payload overflow.
...
func (c *MCPClient) ListStandardsCategories(ctx context.Context, pkg string) string {
	args := map[string]any{}
	if pkg != "" {
		args["package"] = pkg
	}
...
// GetStandard fetches a specific standard entry by exact key from the recall
// standards namespace.
func (c *MCPClient) GetStandard(ctx context.Context, key string) string {
	return c.CallDatabaseTool(ctx, "get", map[string]any{"namespace": "standards", "key": key})
}
...
// Returns true if an approved session containing the plan_hash is found.
func (c *MCPClient) CheckApprovalExists(ctx context.Context, projectID, planHash string) (bool, error) {
	if !c.RecallEnabled() {
...
// OperationalProfile aggregates runtime metrics computed from historical session
// data. Used by primeFromRecall to adaptively tune orchestrator parameters.
...
// GetOperationalProfile queries recall for recent session telemetry and
// computes an aggregated operational profile for runtime self-tuning.
// Returns nil if recall is unavailable or no sessions exist.
func (c *MCPClient) GetOperationalProfile(ctx context.Context, limit int) *OperationalProfile {
	if !c.RecallEnabled() {
		return nil
	}
	...
	raw := c.ListSessionsByFilter(ctx, "", "magictools", "", limit)
	if raw == "" {
		return nil
	}
	...
	for _, entryRaw := range entries {
		...
		// Extract server_id and outcome for failure rate computation
